<?php
 ...
 $gname=$_GET['gname'];
 $gid=$_GET['gid'];
 $assign_answr=$_GET['assign_answr'];
?>
 			...
 			<div class="deptform">
 				...
 				<form action="" method=POST>
                        <?php
                          ...
                          $uid=$_SESSION['userid'];
                          $sql="SELECT assignname,assignmax,tblStudent.studname from tblAssignment INNER JOIN tblGroup ON tblGroup.gid=tblAssignment.assigngid INNER JOIN tblGroupstud ON tblGroupstud.sgid=tblGroup.gid INNER JOIN tblStudent ON tblGroupstud.gstudemail=tblStudent.studemail where tblGroupstud.gstudid='$gstudid' AND gid='$gid' AND assignid='$assignid'";
            ...
            <input type="text" name="txtstud" class="form-control" value="<? echo $row['studname'] ?>" readonly>
                 ...
                 <div class="row">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" name="txttest"  value="<? echo $row['assignname'] ?>" readonly>
                  </div>

                  <div class="form-group">
                    <label>Max Marks</label>
                    <input type="text" name="txtmax"  value="<? echo $row['assignmax'] ?>" readonly>
                  </div>
                </div>
                  ...
                  <div class="col-md-6">
                   <div class="form-group">
                    ...
                    <input type="text" name="txttest" value="<? echo $assign_answr ?>" readonly>
                  </div>
...
<?php
 ...
 if(isset($_POST['subj_submit']))
 {
    ...
    if($scored>$row['assignmax'])
    {
      echo '<script>alert("Invalid Score Entered! (See Maximimum Marks for the Assignment)")</script>';
    }
   else
  {
   ...
   if($r2[0]>0)
    {
      ...
      $sql="UPDATE tblAssignmark set assignmrk='$scored' where assignmrkgid='$gid' AND assignmrkassignid='$assignid' AND assignmrkgstudid='$gstudid'";
      if($conn->query($sql)===TRUE)
      {
        echo '<script>alert("New Score Recorded Successfully");';
        echo 'location.href="teachviewassignresponse2.php?gname='.$gname.'"</script>';
      }
      else
      {
 ...
 {
   $sql="INSERT into tblAssignmark (assignmrkgid,assignmrkassignid,assignmrkgstudid,assignmrk,assignmrkstatus)VALUES('$gid','$assignid','$gstudid','$scored','1')";
 	if($conn->query($sql)===TRUE)
  {
