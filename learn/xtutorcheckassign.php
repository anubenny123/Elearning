 	...
 	<div class="row">
 		...
 		<div class="col-md-6">
 		  <div class="formspace">
 			...
 			<div class="deptform">
 				...
 				<form action="" method=POST>
                        <?php
                          ...
                          $uid=$_SESSION['userid'];
                          $sql="SELECT xassignname,xassignmax,tblXStudent.xstudname from tblXAssignment INNER JOIN tblXGroup ON tblXGroup.xgid=tblXAssignment.xassigngid INNER JOIN tblXGroupstud ON tblXGroupstud.xsgid=tblXGroup.xgid INNER JOIN tblXStudent ON tblXGroupstud.xgstudemail=tblXStudent.xstudemail where tblXGroupstud.xgstudid='$gstudid' AND xgid='$gid' AND xassignid='$assignid'";
            ...
            <input type="text" name="txtstud" class="form-control" value="<? echo $row['xstudname'] ?>" readonly>
                  </div>
                 ...
                 <div class="row">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" name="txttest"  value="<? echo $row['xassignname'] ?>" readonly>
                  </div>

                  <div class="form-group">
                    <label>Max Marks</label>
                    <input type="text" name="txtmax"  value="<? echo $row['xassignmax'] ?>" readonly>
                  </div>
                </div>
...
<?php
   ...
   if($r2[0]>0)
    {
      ...
      $sql="UPDATE tblXAssignmark set xassignmrk='$scored' where xassignmrkgid='$gid' AND xassignmrkassignid='$assignid' AND xassignmrkgstudid='$gstudid'";
      if($conn->query($sql)===TRUE)
      {
        echo '<script>alert("New Score Recorded Successfully");';
        echo 'location.href="xtutorviewassignresponse2.php?gname='.$gname.'"</script>';
      }
      else
      {
 ...
 {
   $sql="INSERT into tblXAssignmark (xassignmrkgid,xassignmrkassignid,xassignmrkgstudid,xassignmrk,xassignmrkstatus)VALUES('$gid','$assignid','$gstudid','$scored','1')";
 	if($conn->query($sql)===TRUE)
  {
