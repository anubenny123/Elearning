 	...
 	<div class="row">
 		...
 		<div class="col-md-6">
 		  <div class="formspace">
 		    <br><br>
 			<div class="deptform">
 			    <b><h1>Delete Works Attempted </h1></b>
             ...
             <br>
 				     <div class="form-group">
                       ...
                       <option value="none" selected>--Works Expired--</option>
                       <?php
                        ...
                        $cdateandtime=date('Y-m-d').date('h:i:s');                       
                          ...
                          $sql="SELECT assignid,assignname from tblAssignment where assigngid='$grpr2[0]'  AND assignstatus=1  AND CONCAT(assigndate,assigntime)<'$cdateandtime'";
                          $result=$conn->query($sql);
                          while($row=mysqli_fetch_array($result))
                          {
                        echo '<option value="'.$row['assignid'].'">'.$row['assignname'].'</option>';
                          }
...
<script>
      ...
      //alert(stopic);
      $.ajax({
      url: "getdatetodelassign.php?gettopicid="+stopic,
      type:'POST',
...
<?php
 ...
 else
 {
  ...
  $uid=$_SESSION['userid'];
 	$sql="UPDATE tblAssignment set assignstatus=-1 where assignid='$topicforassigndel' AND assigngid='$grpr2[0]' AND assignstatus=1";
 	if($conn->query($sql)===TRUE)
 		echo '<script>var choice=confirm("One Event Deleted Successfully.Do You want to Create a new Assignment?");if(choice==true){location.href="teachaddassign.php?gname='.$gname.'"}else{ location.href="insidegrp.php?gname='.$gname.'"}</script>';
