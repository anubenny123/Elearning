 				     ...
 				     <div class="form-group">
                       ...
                       <?php
                        ...
                        $grpr2=mysqli_fetch_array($grpresult);
                          ...
                          $sql="SELECT mtrlid,mtrlname from tblMaterial where mtrlstatus=1 AND mtrlgid='$grpr2[0]'";
                          $result=$conn->query($sql);
                          while($row=mysqli_fetch_array($result))
                          {
                        echo '<option value="'.$row['mtrlid'].'">'.$row['mtrlname'].'</option>';
                          }
...
<?php
 ...
 else
 {
  ...
  $uid=$_SESSION['userid'];
 	$sql="UPDATE tblMaterial set mtrlstatus=-1 where mtrlid='$topictomtrldel' AND mtrlgid='$grpr2[0]' AND mtrlstatus=1";
 	if($conn->query($sql)===TRUE)
 		echo '<script>var choice=confirm("One Material Deleted Successfully.Do You want to Delete More?");if(choice==true){location.href="teachdelmtrl.php?gname='.$gname.'"}else{ location.href="insidegrp.php?gname='.$gname.'"}</script>';
