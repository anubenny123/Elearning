 	...
 	<div class="row">
 		...
 		<div class="col-md-6">
 		  <div class="formspace">
 		    <br><br>
 			<div class="deptform">
 			    <b><h1>Delete Pending Tests</h1></b>
 				<form action="" method=POST>
             <br>
 				     <div class="form-group">
  	  				<label>Select Test Topic to Delete</label>
  	  				<select name="txttopicfortestdel" id="topictodel" class="form-control" required>
                       <option value="none" selected>--Recently Sheduled Tests--</option>
                       <?php
                        ...
                        $cdateandtime=date('Y-m-d').' '.date('H:i:s');
                          ...
                          $sql="SELECT testid,testname from tblTest where testgid='$grpr2[0]'  AND teststatus=1 AND CONCAT(CONCAT(testdate,' '),teststarttime) >='$cdateandtime'";
                          $result=$conn->query($sql);

                          while($row=mysqli_fetch_array($result))
                          {
                            //echo '<script>alert("'.$row['testdate'].$row['teststarttime'].'")</script>';
              ...
              <div class="form-group">
              <br>
  	  				<input type="submit" name="testdelsubmit" class="btn btn-primary" value="Delete Event">&nbsp&nbsp	
...
<?php
 ...
 {
  $topicfortestdel=$_POST['txttopicfortestdel'];

  if($topicfortestdel=="none")
 ...
 else
 {
  ...
  $uid=$_SESSION['userid'];
 	$sql="UPDATE tblTest set teststatus=-1 where testid='$topicfortestdel' AND testgid='$grpr2[0]' AND teststatus=1";
 	if($conn->query($sql)===TRUE)
 		echo '<script>var choice=confirm("One Event Deleted Successfully.Do You want to Create a new Test Event?");if(choice==true){location.href="teachaddnewtest.php?gname='.$gname.'"}else{ location.href="insidegrp.php?gname='.$gname.'"}</script>';
